...
import { Truck, Shield, Zap, Star, ShoppingCart, ArrowRight, MapPin, Phone, Mail, Monitor, FileText, Armchair, Package } from "lucide-react";
...
const featuredProducts = [
  { name: "Ergonomic Office Chair", rating: 4.8, reviews: 128, price: 299, emoji: "🪑" },
  { name: "Wireless Mouse", rating: 4.6, reviews: 128, price: 49, emoji: "🖱️" },
  { name: "Desk Organizer Set", rating: 4.9, reviews: 128, price: 35, emoji: "📋" },
  { name: "LED Desk Lamp", rating: 4.7, reviews: 128, price: 79, emoji: "💡" },
];

const stats = [
  { value: "2000+", label: "Products Available", sub: "Curated for your business" },
    ...
    <div className="min-h-screen bg-background overflow-y-auto">
      ...
      <section className="relative py-16 sm:py-24 px-4 sm:px-8 text-center bg-gradient-to-b from-secondary to-background">
        ...
        <AnimatedSection delay={300}>
          <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto mb-10">
            Your trusted B2B platform for daily-use items, office supplies, and logistics solutions.
          </p>
        </AnimatedSection>
        <AnimatedSection delay={400}>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
            <Button size="lg" className="w-full sm:w-auto px-8 py-3 text-base hover:scale-[1.02] active:scale-[0.98] transition-transform" onClick={() => navigate("/categories")}>
              Browse Shop
            </Button>
            <Button size="lg" variant="outline" className="w-full sm:w-auto px-8 py-3 text-base border-primary text-primary hover:bg-primary/5 hover:scale-[1.02] active:scale-[0.98] transition-transform">
              Learn More
            </Button>
          </div>
        </AnimatedSection>
      ...
      <section className="py-8 sm:py-12 px-4 sm:px-8">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          {[
            { icon: Truck, title: "Free Shipping", desc: "On orders over ₹7,999" },
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-8 bg-secondary/50">
        <AnimatedSection>
          <div className="max-w-6xl mx-auto text-center mb-8 sm:mb-10">
            <h3 className="text-xl sm:text-2xl font-bold text-foreground mb-2">Shop by Category</h3>
            <p className="text-muted-foreground text-sm sm:text-base">Browse our extensive collection of business essentials</p>
          </div>
        </AnimatedSection>
            ...
            <AnimatedSection key={cat.name} delay={i * 80}>
              ...
              >
                <CardContent className="p-4 sm:p-6 text-center">
                  <div className="text-4xl sm:text-5xl mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300">{cat.emoji}</div>
                  <p className="font-semibold text-foreground mb-1 text-sm sm:text-base">{cat.name}</p>
                  <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4">{cat.count} products</p>
                  <span className="inline-flex items-center gap-1 text-xs sm:text-sm font-medium text-primary group-hover:gap-2 transition-all duration-300">
                    Browse <ArrowRight className="h-3.5 w-3.5 sm:h-4 sm:w-4 group-hover:translate-x-0.5 transition-transform" />
                  </span>
                </CardContent>
              </Card>
            </AnimatedSection>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-8">
        <AnimatedSection>
          <div className="max-w-6xl mx-auto text-center mb-8 sm:mb-10">
            <h3 className="text-xl sm:text-2xl font-bold text-foreground mb-2">Featured Products</h3>
            <p className="text-muted-foreground text-sm sm:text-base">Hand-picked items for your business needs</p>
          </div>
        </AnimatedSection>
        <div className="max-w-6xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              ...
              <Card className="bg-card border border-border hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden">
                <CardContent className="p-0">
                  ...
                  <div className="p-4 sm:p-5">
                    <p className="font-semibold text-foreground mb-2 text-sm sm:text-base line-clamp-1">{p.name}</p>
                    <div className="flex items-center gap-1 mb-3">
                      <Star className="h-3.5 w-3.5 sm:h-4 sm:w-4 fill-warning text-warning" />
                      <span className="text-xs sm:text-sm font-medium text-foreground">{p.rating}</span>
                      <span className="text-[10px] sm:text-xs text-muted-foreground">({p.reviews})</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-lg sm:text-xl font-bold text-primary">₹{p.price}</span>
                      <Button
                        size="sm"
                        variant="outline"
            ...
            <AnimatedSection key={s.label} delay={i * 100}>
              <Card className="bg-card border border-border text-center hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                <CardContent className="p-6 sm:p-8">
                  <p className="text-3xl sm:text-4xl font-bold text-primary mb-2">{s.value}</p>
                  <p className="font-semibold text-foreground mb-1 text-sm sm:text-base">{s.label}</p>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-8">
        <AnimatedSection>
          <div className="max-w-6xl mx-auto rounded-2xl bg-primary/15 p-8 sm:p-12 text-center">
            <h3 className="text-2xl sm:text-3xl font-bold text-foreground mb-3">Ready to Get Started?</h3>
            <p className="text-muted-foreground mb-6 sm:mb-8 max-w-lg mx-auto text-sm sm:text-base">
              Join hundreds of businesses that trust NidoTech for their supply needs.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
              <Button size="lg" className="w-full sm:w-auto hover:scale-[1.02] active:scale-[0.98] transition-transform" onClick={() => navigate("/categories")}>Browse Products</Button>
              <Button size="lg" variant="outline" className="w-full sm:w-auto hover:scale-[1.02] active:scale-[0.98] transition-transform">Contact Sales</Button>
            </div>
          </div>
        </AnimatedSection>
      </section>
      ...
      <footer className="border-t border-border bg-secondary/30 py-8 sm:py-12 px-4 sm:px-8">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-10">
          <div>
            <h4 className="font-bold text-foreground text-lg mb-1">NIDO-TECHNOLOGY</h4>
            <p className="text-sm text-primary mb-4">Premium E-Commerce Excellence</p>
          </div>
          <div>
            <h4 className="font-semibold text-foreground mb-3">Quick Links</h4>
            <ul className="space-y-2 text-sm text-muted-foreground">
              {["About Us", "Products", "Services", "Contact", "Blog"].map((l) => (
                <li key={l} className="hover:text-primary hover:translate-x-1 cursor-pointer transition-all duration-200">{l}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-3 text-sm text-muted-foreground">
              <li className="flex items-start gap-2"><MapPin className="h-4 w-4 shrink-0 mt-0.5" />#41/1, 2nd Floor, 10th Cross, 11th Main, Wilsongarden, Bengaluru - 560027</li>
              <li className="flex items-center gap-2"><Phone className="h-4 w-4 shrink-0" />+91 99833983</li>
              <li className="flex items-center gap-2"><Mail className="h-4 w-4 shrink-0" />nidotechnologies@gmail.com</li>
            </ul>
          </div>
        </div>
