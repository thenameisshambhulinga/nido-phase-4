    ...
    <div className="min-h-screen flex bg-background">
      ...
      <div className="hidden lg:flex lg:w-[55%] relative overflow-hidden bg-sidebar text-sidebar-foreground flex-col justify-between p-10">
        ...
        <div className="relative z-10">
          ...
          <div
            className={`mt-12 transition-all duration-700 delay-200 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
            ...
            <p className="text-sm text-sidebar-foreground/60 mt-3 max-w-sm leading-relaxed">
              Manage vendors, automate approvals, track spending — all from one
              unified platform.
            </p>
          </div>
        </div>
        ...
        {/* Workflow steps */}
        <div
          className={`relative z-10 transition-all duration-700 delay-500 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        >
          ...
          <div className="flex gap-3">
            {WORKFLOW_STEPS.map((step, i) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`h-5 w-5 mb-2 transition-colors ${isActive ? "text-sidebar-primary" : "text-sidebar-foreground/40"}`}
                  />
                  <p
                    className={`text-xs font-semibold mb-0.5 ${isActive ? "text-sidebar-primary-foreground" : "text-sidebar-foreground/70"}`}
                  >
                    {step.title}
                  </p>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10">
        <div
          className={`w-full max-w-md transition-all duration-700 delay-300 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        >
          ...
          <div className="lg:hidden text-center mb-8">
            ...
            <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground font-medium mt-1">
              CorpEssentials
            </p>
                ...
                <TabsContent value="login" className="mt-0">
                  <form onSubmit={handleLogin} className="space-y-4">
                    ...
                    <div className="mt-4 rounded-lg border border-dashed border-border/70 bg-muted/30 p-4 text-sm text-muted-foreground">
                      User creation is owner-only. Log in with the owner account
                      to create client admin and client user accounts.
                    </div>
