...
import { Upload, FileSpreadsheet, AlertTriangle, CheckCircle2, XCircle, Download, ArrowRight, ArrowLeft, Loader2 } from "lucide-react";
import {
  parseVendorUpload,
  parseClientUpload,
...
interface BulkUploadModalProps {
  ...
  onClose: () => void;
  type: UploadType;
  existingRecords?: { id: string; name: string; contactEmail?: string; email?: string }[];
  onSuccess: (results: BulkUploadResult) => void;
}

const TYPE_LABELS: Record<UploadType, string> = { vendor: "Vendors", product: "Products", client: "Clients" };

export default function BulkUploadModal({ open, onClose, type, existingRecords = [], onSuccess }: BulkUploadModalProps) {
  ...
  const handleConflictAction = (rowIndex: number, action: "update" | "skip" | "create") => {
    setRows(prev => prev.map(r => r.rowIndex === rowIndex ? { ...r, conflictAction: action } : r));
  };
  ...
  const handleImport = () => {
    ...
    const res: BulkUploadResult = { created, updated, skipped, errors: allErrors, records: importedRecords };
    setResult(res);
    setStage("results");
    onSuccess(res);
  };
  ...
  const stageIndex = ["upload", "preview", "conflicts", "results"].indexOf(stage);
      ...
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs text-muted-foreground">
            {["Upload File", "Preview Data", "Resolve Conflicts", "Results"].map((label, i) => (
              <span key={label} className={i <= stageIndex ? "text-primary font-medium" : ""}>{label}</span>
            ))}
          </div>
        ...
        {stage === "upload" && (
          <div className="flex-1 flex flex-col items-center justify-center gap-6 py-8">
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">or click to browse — .xlsx, .xls, .csv accepted</p>
                </>
              )}
              <input
                ref={fileInputRef}
            ...
            <Button variant="outline" className="gap-2" onClick={handleDownloadTemplate}>
              <Download className="h-4 w-4" /> Download {TYPE_LABELS[type]} Template
            </Button>
          </div>
        )}
        ...
        {stage === "preview" && (
          <div className="flex-1 flex flex-col gap-4 min-h-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <Badge className="bg-primary/10 text-primary border-primary/30" variant="outline">{rows.length} rows</Badge>
              ...
              {totalErrors > 0 && (
                <Badge className="bg-destructive/10 text-destructive border-destructive/30" variant="outline">
                  <XCircle className="h-3 w-3 mr-1" />{totalErrors} errors
                </Badge>
              )}
              {totalWarnings > 0 && (
                <Badge className="bg-warning/10 text-warning border-warning/30" variant="outline">
                  <AlertTriangle className="h-3 w-3 mr-1" />{totalWarnings} warnings
                </Badge>
              )}
            </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {rows[0] && Object.keys(rows[0].data).slice(0, 6).map(key => (
                      <TableHead key={key} className="min-w-[120px]">{key.replace(/_/g, " ")}</TableHead>
                    ))}
                ...
                <TableBody>
                  {rows.map(row => (
                    <TableRow key={row.rowIndex} className={row.errors.length > 0 ? "bg-destructive/5" : row.warnings.length > 0 ? "bg-warning/5" : ""}>
                      ...
                      {Object.values(row.data).slice(0, 6).map((val, idx) => (
                        <TableCell key={idx} className="text-xs max-w-[150px] truncate">{val || "—"}</TableCell>
                      ))}
                      <TableCell className="text-xs">
                        {[...row.errors, ...row.warnings].map((e, idx) => (
                          <span key={idx} className={`block ${e.severity === "error" ? "text-destructive" : "text-warning"}`}>
                            {e.field}: {e.message}
                          </span>
                        ))}
                      </TableCell>
            ...
            <div className="flex justify-between">
              ...
              {conflicts.length > 0 ? (
                <Button onClick={() => setStage("conflicts")} className="gap-2">
                  Resolve {conflicts.length} Conflict{conflicts.length > 1 ? "s" : ""} <ArrowRight className="h-4 w-4" />
                </Button>
              ) : (
                <Button onClick={handleImport} className="gap-2" disabled={validCount === 0}>
                  Import {validCount} Record{validCount !== 1 ? "s" : ""} <ArrowRight className="h-4 w-4" />
                </Button>
              )}
            </div>
          </div>
        ...
        {stage === "conflicts" && (
          <div className="flex-1 flex flex-col gap-4 min-h-0">
            <p className="text-sm text-muted-foreground">
              {conflicts.length} record{conflicts.length > 1 ? "s" : ""} match existing data. Choose how to handle each:
            </p>
                ...
                <TableBody>
                  {conflicts.map(row => (
                    <TableRow key={row.rowIndex} className="bg-warning/5">
                      ...
                      <TableCell className="font-medium">{row.data.company_name}</TableCell>
                      <TableCell className="text-muted-foreground text-sm">{row.data.email}</TableCell>
                      <TableCell>
                        <Badge variant="outline" className="border-warning text-warning">
                          {row.conflict === "duplicate_email" ? "Email exists" : "Name match"}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Select value={row.conflictAction || "skip"} onValueChange={(v) => handleConflictAction(row.rowIndex, v as "update" | "skip" | "create")}>
            ...
            <div className="flex justify-between">
              <Button variant="outline" onClick={() => setStage("preview")} className="gap-2">
        ...
        {stage === "results" && result && (
          <div className="flex-1 flex flex-col items-center justify-center gap-6 py-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full max-w-lg">
              {[
                { label: "Created", value: result.created, color: "text-success" },
                { label: "Updated", value: result.updated, color: "text-primary" },
            ...
            {result.errors.length > 0 && (
              <div className="w-full max-w-lg border rounded-lg p-3 bg-destructive/5">
                ...
                {result.errors.slice(0, 5).map((err, i) => (
                  <p key={i} className="text-xs text-muted-foreground">Row {err.row}: {err.field} — {err.message}</p>
                ))}
                {result.errors.length > 5 && <p className="text-xs text-muted-foreground mt-1">...and {result.errors.length - 5} more</p>}
              </div>
            )}
